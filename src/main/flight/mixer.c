...
void writeMotors(void)
{
    if (pwmAreMotorsEnabled()) {
        ...
        updateBatteryRestingVoltage(absMotorOutputSum / (motorOutputRange * motorCount));
    }
}
...
static float applyThrottleCurve(float throttle)
{
    if (currentControlRateProfile->thrust_linearization_level) {
        if (!currentControlRateProfile->throttle_linearization) {
            ...
            return throttle *
                   scaleRangef(currentControlRateProfile->thrust_linearization_level, 0, 100, 1.0f, throttle);
        }
